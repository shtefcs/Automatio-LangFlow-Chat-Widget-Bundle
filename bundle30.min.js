(() => {
  // Import and initialize the marked library with better error handling
  async function loadMarked() {
    if (window.marked && typeof window.marked === 'function') return window.marked;
      ...
      function tryLoadScript() {
        ...
        script.onload = () => {
          if (window.marked && typeof window.marked === 'function') {
            ...
            if (loadAttempt < cdnUrls.length) {
              ...
              reject(new Error('Could not initialize marked library'));
            }
          }
        };
        ...
        script.onerror = () => {
          ...
          if (loadAttempt < cdnUrls.length) {
            ...
            reject(new Error('Failed to load marked from all CDN sources'));
          }
        };
  ...
  // Enhanced markdown renderer with better fallback
  async function renderMarkdown(text) {
    ...
    try {
      ...
      // Enhanced fallback with basic markdown-like formatting
      return text
        .replace(/&/g, '&amp;')
        .replace(/</g, '&lt;')
        .replace(/>/g, '&gt;')
        // Links with titles [text](url)
        .replace(/\[([^\]]+)\]$$([^)]+)$$/g, '<a href="$2" target="_blank" rel="noopener noreferrer" class="chat-link">$1</a>')
        // Plain URLs
        .replace(/\bhttps?:\/\/[^\s<>[\]]+/g, '<a href="$&" target="_blank" rel="noopener noreferrer" class="chat-link">$&</a>')
        // Lists
  ...
  class LangFlowChat extends HTMLElement {
    ...
    async sendMessage() {
      ...
      try {
        ...
        this.hideTypingIndicator();
        await this.addMessage('assistant', 'Sorry, there was an error sending your message.');
      }
    }
