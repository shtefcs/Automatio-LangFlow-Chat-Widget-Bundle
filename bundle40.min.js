(() => {
  // Improved markdown parser with proper escaping and link cards
  function parseMarkdown(text) {
    ...
    // Custom link format with link cards
    html = html.replace(/\[([^\]]+)\]\$\$([^)]+)\$\$/g, (_, title, url) => {
    ...
    html = html.replace(/!\[([^\]]*)\]\$\$([^)]+)\$\$/g, (_, alt, src) => 
      `<img src="${src}" alt="${alt}" class="message-image" onclick="this.getRootNode().host.showImageModal('${src}', '${alt}')">`
    );
    ...
    html = html.split('\n\n').map(para => {
      ...
      if (
        para.includes('<ul>') || 
        para.includes('<pre>') || 
        para.includes('<div class="link-card"') ||
        para.includes('<img')
      ) {
        return para;
      }
    ...
    addMessage({ role, content }) {
      ...
      if (links.length > 0) {
        ...
        sourcesSection.innerHTML = `
          ...
          ${links.map(link => 
            `<a href="${link.url}" target="_blank" rel="noopener noreferrer" class="source-link">${link.title}</a>`
          ).join('')}
        `;
