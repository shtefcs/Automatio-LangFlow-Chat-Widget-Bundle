...
function createMarkdownParser() {
  ...
  return function parseMarkdown(text) {
    ...
    const patterns = [
      // Links with link cards - must be processed first
      {
        pattern: /\[([^\]]+)\]\$\$([^$]+?)\$\$/g,
        replace: (_, title, url) => createLinkCard(title.trim(), url.trim())
      },
      // Images
      {
        pattern: /!\[([^\]]*)\]\$\$([^$]+?)\$\$/g,
      ...
      {
        pattern: /```([\s\S]+?)```/g,
        replace: (_, code) => `<pre><code>${code.trim()}</code></pre>`
      },
      // Inline code
      {
        pattern: /`([^`]+)`/g,
    ...
    html = html.split('\n\n').map(block => {
      ...
      if (
        block.includes('<ul>') ||
        block.includes('<pre>') ||
        block.includes('class="link-card"') ||
        block.includes('<img')
      ) {
        return block;
      }
