  ...
  function renderMarkdown(text) {
    ...
    const processedLines = lines.map(line => {
      ...
      if (line.trim() === '') {
        if (inList) {
          inList = false;
          const list = `<${listType}>${listItems.join('')}</${listType}>`;
          listItems = [];
          return list;
        }
        return '<br>';
      ...
      // Links - handle both [text](url) format and plain URLs
      line = line
        .replace(/\[([^\]]+)\]$$([^)]+)$$/g, '<a href="$2" target="_blank" rel="noopener noreferrer" class="chat-link">$1</a>')
        .replace(/(?<![\[$$])(https?:\/\/[^\s<>]+)(?![\]$$])/g, '<a href="$1" target="_blank" rel="noopener noreferrer" class="chat-link">$1</a>');
      ...
      // Code blocks (```code```)
      line = line.replace(/```([^`]+)```/g, '<pre><code>$1</code></pre>');
    ...
    // Join lines and clean up empty lines between list items
